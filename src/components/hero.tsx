...
function FloatingText({ yShift = 0, children, ...props }) {
	const ref = useRef();
	const isMobile = useMobile();
	...
	useFrame((state) => {
		if (ref.current) {
			...
			ref.current.position.y = Math.sin(t / 2) / 15 - yShift;
		}
	});
		...
		>
			...
			>
				...
				>
					...
					{isMobile ? null : (
						<FloatingText
							position={[0, isMobile ? 1.3 : 2.2, isMobile ? -1.1 : -2.5]}
							yShift={isMobile ? -0.5 : -2}
						>
							FULL STACK
						</FloatingText>
					)}
					<FloatingText
						position={[0, isMobile ? 1.3 : 2.2, isMobile ? -0.8 : -1.8]}
						yShift={0}
					>
						DEVELOPER
					</FloatingText>
					...
					>
						...
						>
							<h1
								...
								} font-bold text-white mb-2 tracking-wider`}
							>
								Mancoba Thabethe
							</h1>
							<p
								...
								} mb-4 lg:font-semibold`}
							>
								Full Stack Developer <br /> based in Nelspruit, South Africa
							</p>
